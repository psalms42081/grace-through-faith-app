// Path B Brief 03 Phase A — Sabbath School v2 (hidden route).
// Light-first rebuild of app/sabbath-school.tsx. NO route swap yet:
// reached only via the preview pill on the current screen (/sabbath-school).
// Rollback: this file is unrouted from nav; the old screen remains canonical.
              ...
              {pastQuarters.length === 0 ? (
                ...
                pastQuarters.map((q) => (
                  <Pressable
                    ...
                    onPress={() =>
                      router.push(
                        `/sabbath-school-quarter-v2?quarterCode=${(q as any).quarterCode}&title=${encodeURIComponent(q.title)}` as any
                      )
                    }
