...
interface SdaPlanData {
  plan: SdaPlanMeta;
  days: SdaDayData[];
}

const PLANS: SdaPlanData[] = [
  {
    plan: {
...
export async function seedSdaDevotionals(db: NodePgDatabase<any>): Promise<void> {
...
const isMain = process.argv[1] != null &&
  (process.argv[1].endsWith("/seed-sda-devotionals.ts") ||
   process.argv[1].endsWith("\\seed-sda-devotionals.ts"));
if (isMain) {
  runCli().catch((err) => {
