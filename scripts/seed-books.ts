...
async function seed() {
  console.log("Seeding Bible translation...");
  ...
  for (const book of BOOKS) {
    await db.insert(bibleBooks).values(book).onConflictDoNothing();
  }

  console.log("Seed complete! 66 books + KJV translation inserted.");
  await pool.end();
}
