...
- **Home Screen Visual Identity:** Premium image-based cards replacing icons throughout the Home screen. "Your Daily Rhythm" (Read/Study/Pray) uses full-width image banner cards with LinearGradient overlays (`assets/home-cards/read.png`, `study.png`, `pray.png`). DevotionalCard uses `study-guide.png` background. Connect page uses atmospheric image cards (`assets/connect-cards/`). Design follows warm lighting, natural colors, shallow depth of field, calm/reverent tone across all imagery.
