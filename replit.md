...
- **Interactive AI Storyteller (Pause & Wonder):** Kids stories are now interactive "Choose Your Own Adventure" experiences. Every 3 paragraphs, an AI-generated "Pause & Wonder" card appears inline with a warm, imaginative question and 3 emoji-based multiple-choice answers. Answering triggers `expo-haptics` `notificationAsync(Success)` and awards +10 points to child profile. Wonder moments are cached per story+ageGroup in `kids_wonder_cache` table. Answer state stored in `kidsProgress.wonderAnswers` (jsonb array of moment indices). Points only awarded once per unique moment (prevents double-counting). UI hydrates answered state from server on revisit. AI prompt adapts to age group (little_lambs=4-7 simple, young_disciples=8-12 moderate, young_disciples_plus=13-17 deeper). Screen: `app/kids-story/[id].tsx` (legacy inline reader, kept as fallback). Story text rendered in large Lora serif font paragraph-by-paragraph. Endpoints: `GET /api/kids/stories/:id/wonder?ageGroup=`, `POST /api/kids/wonder/answer`. AI function: `generatePauseAndWonder` in `server/services/ai-engine.ts`.
- **Kids Story Engine (Scene-Based Reader):** A new immersive scene-based storybook reader at `app/kids/story/[id].tsx`. AI generates 5-7 scenes per story via `generateStoryScenes()` (gpt-4o-mini), each with narration, watercolor illustration prompt, and optional Socratic Pause & Wonder question. Scenes are persisted in `kids_story_scenes` table (storyId, sceneIndex, narration, illustrationPrompt, pauseAndWonder jsonb). Horizontal FlatList pager with unique gradient backgrounds per scene. Word-by-word TTS highlighting via expo-speech with timer-based word tracking. BlurView overlay for Pause & Wonder questions with emoji-based options, confetti burst on answer, haptic feedback. Story completion awards 25 Seed Points (auto-looks up child profile for user), recalculates level (totalPoints/100+1), triggers Level Up modal and streak celebration. Navigation from kids-stories tab and home daily story card now routes to scene reader. Endpoints: `GET /api/kids/story/:id/scenes`, `POST /api/kids/story/:id/generate`, `POST /api/kids/story/award-points`. AI function: `generateStoryScenes` in `server/services/ai-engine.ts`.
