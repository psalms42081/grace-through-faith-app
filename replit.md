...
- **4-Layer Study Model:** Integrates Bible text (KJV, ASV, WEB) with historical context, classic commentaries, and AI-generated application content. Includes a segmented progress bar (Text / Context / Insight / Transformation), per-layer completion tracking stored in `layer_completions` table (per user, book, chapter, layer), "Mark Layer Complete" / "Next Layer" CTA buttons, and per-book layer completion summary (% bars) in the Bible Knowledge Map tooltip on the Profile tab. Layer 3 (Insight) has structured journal prompts: Theological Themes, Revelation of God, Revelation of Humanity, Biblical Narrative Connection. Layer 4 (Transformation) has structured journal prompts: Belief Challenged, Habit Shaped, Conversation Impacted, Prayer Response (with Save to Prayer Journal link). All journal entries persisted in `study_journal_entries` table (per user, book, chapter, layer, sectionKey). Per-section gold checkmark completion indicators. Gentle note for L3/L4: "Depth increases when you write a response" shown when marking reflective layers complete without entries. **Study Depth Indicator** displays subtle label (Emerging / Developing / Established) above the progress bar — Emerging = any engagement, Developing = L1-L2 complete + 2+ Insight prompts, Established = all 4 layers complete + all 8 prompts saved. **Next Best Step** hint below progress bar shows dynamic guidance (e.g., "Continue Insight: 2 prompts remaining"). Deep Study Session mode: guided 4-layer sequence (Text -> Context -> Insight -> Transformation) with session progress dots, estimated time, pause/resume via `pausedLayerIndex` state + `deepSessionRef` to avoid stale closures, and end-of-session summary showing completed layers, saved reflections (collapsed/expandable), and "Save Prayer Response" CTA. Session state persisted in AsyncStorage (`@grace-through-faith/deep-session`). **Study Summary Artifact**: on session completion, a collapsible "Study Summary" card shows a formatted plain-text summary (reference, layers completed, depth label, filled insight/transformation entries, prayer response) with "Copy Summary" (expo-clipboard) and native "Share" (expo-sharing + expo-file-system) buttons. Summary generation is client-side only using existing fetched journal maps. **Revisit Your Reflections** section on Profile tab shows recent chapters with saved journal entries, excerpts (120 chars), last-edited date, and tap-to-navigate back to that chapter's study layer. Backend endpoint: `GET /api/study-journal/revisit?userId=` with batch book name lookup.
