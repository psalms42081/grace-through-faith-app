...
- **Live Fellowship:** Community feature for structured group experiences with real-time video/audio via LiveKit. Group creators are automatically assigned the "leader" role. Leaders can promote/demote members and remove them via an action sheet UI. Auto-repair ensures creators missing from the membership table are re-added as leaders on group detail fetch. Screen sharing available for leaders/moderators (web only; uses `getDisplayMedia` API). Stale sessions (>6hrs) auto-cleaned on active streams fetch. Room HTML served from `server/templates/livekit-room.html` via `process.cwd()` path resolution. Max 50 participants per room (LiveKit config). Stream viewer at `app/stream/[id].tsx` uses WebView (mobile) or iframe (web) embedding the LiveKit room page.
...
- **Admin User Management:** Admin dashboard (admin-review.tsx) includes "Users" and "Approvals" tabs for user management. Users tab: paginated list with search by name/email, role filter chips, user detail modal with groups/activity/posts, and role change (member/elder/admin). Leader Approvals tab: pending leader access requests with approve/reject actions. Backend routes in `server/routes/admin-users.ts` (5 endpoints: list users, user detail, update role, list leader requests, review leader request). Schema: `leader_requests` table in `shared/schema.ts`.
- **Content Engine & Study Resources:** Two-stage content pipeline with review workflow and versioning.
- **Leader Access Pipeline:** Full request-to-approval pipeline for church leader access. Non-leader users see "Request Leader Access" in Settings, which opens a form (full name, church name, role, contact email, description). Requests stored in `leader_requests` table. Admins see a "Leaders" tab in the Admin Dashboard with filtering and Approve/Reject actions. Approval updates user role to `church_leader` and sends push notification. Approved leaders see "Leader Tools" section in Settings with Broadcast Announcements (`app/leader-broadcast.tsx`) and Church Member Analytics (`app/leader-analytics.tsx`). Backend endpoints in `server/routes/community.ts`. Push notifications via `notifyUser()` in `server/services/push-notifications.ts`.
