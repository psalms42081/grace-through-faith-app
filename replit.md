...
- **Sabbath School Companion Lessons:** Published companion lessons surface directly in the Sabbath School flow. Primary: Companion Lesson card on the weekly lesson screen (between days grid and Discussion Mode). Secondary: "This Week's Companion Lesson" card on Home tab. Archive: purple Companion badge on lesson cards in past quarter view. Backend API (`/api/sabbath-school/current` and `/quarter/:quarterCode`) includes companion data when a published `sabbath-school-companion` resource exists for a lesson, linked via `sourceRef.lessonId`.
