...
- **Sabbath School Mode:** Weekly-synced Sabbath School lesson engine powered by Adventech's open-source quarterly content (GitHub: `Adventech/sabbath-school-lessons`). Backend sync service (`server/services/sabbath-school-sync.ts`) fetches current quarter data daily from `raw.githubusercontent.com`, parsing YAML metadata and markdown lesson content into 5 database tables (`sabbath_school_quarterly`, `sabbath_school_lesson`, `sabbath_school_day`, `sabbath_school_user_progress`, `sabbath_school_discussion_prep`). API routes at `server/routes/sabbath-school.ts` serve current lesson with day-level progress tracking and AI-generated discussion prep (cached). Frontend: main hub (`app/sabbath-school.tsx`) with quarterly info, 7 day cards with completion tracking, and Discussion Mode button; day reader (`app/sabbath-school-day.tsx`) with markdown rendering, journal, and completion; discussion companion (`app/sabbath-school-discussion.tsx`) with AI-generated questions, summary, and reflection prompts. Entry points on Home screen ("This Week's Lesson" card with progress) and Sabbath Experience Worship Pathways.
