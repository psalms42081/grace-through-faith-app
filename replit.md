...
- **Content Engine & Resources Library:** Two-stage content pipeline architecture:
  ...
  - **Diff View:** `resources.previous_content_json` JSONB column preserves old content on regeneration. Batch generator captures contentJson before deleting old resource and stores it in the new resource's `previousContentJson`. `GET /api/admin/pipeline/resource/:id/diff` (requireEditor) returns section-by-section comparison (overview, daily study prompts, discussion questions, memory verse guide, family worship, EGW connections) with status badges (changed/unchanged/added/removed), generation metadata diff (prompt version, model, tokens), and summary counts. UI shows diff button (git-compare icon, yellow) on items with previous versions, opening a full-screen diff modal with collapsible sections — changed sections expanded by default, stacked old/new content blocks.
