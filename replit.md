...
- **Inductive Study Progression Engine:** The Socratic Study Guide (`app/study-guide.tsx`) tracks real learning progress through Observe, Interpret, and Apply stages. Progression is stored in a `progression` JSON column on `study_guide_session` with per-stage tracking (responses, meaningfulCount, categories, completedAt). Stage advancement requires meaningful engagement: Observe needs 2 distinct observation categories (speaker, structure, titles, etc. via `inferObserveCategory()`), Interpret needs 2 meaningful responses, Apply needs 1 personal reflection (validated by `hasPersonalReflection()` requiring first-person language + action intent). Verse-copy detection (`isVerseCopy()`) prevents copied text from counting. AI evaluation tags ([MEANINGFUL]/[SHALLOW]/[OFF_TOPIC]/[CAT:xxx]) with robust heuristic fallback via `parseEvaluationTag()`. Stage transitions trigger AI-generated bridge messages. On completion, `generateStudySummary()` creates a 3-part personalized summary (observed/reflected/takeaway) stored in `summary` column. Completed studies are guarded against further responses. Progression feeds into the Engage ring via existing `study_guide_session` query.
...
- **Verse Action Tools:** Bottom-sheet verse tools (Copy, Highlight, Bookmark, Words, Insight, Verse Map, Guided Study) navigate to standalone stack routes (`/word-study`, `/historic-voices`, `/verse-map`, `/study-guide`, `/passage-context`) for proper Android back-button behavior. All tools available to all users (no pro gate).
- **Insight & Voices (`app/historic-voices.tsx`):** Commentary screen with Adventist-first content ordering. Ellen G. White AI-generated Adventist perspective shown first with gold left border, star badge, and egwwritings.org link. Classic commentators (Matthew Henry, JFB, Adam Clarke, John Gill) follow. Filter chips with solid accent background when active. EGW content is AI-generated via `generateEgwInsight()` in `server/routes/study.ts`, cached to prevent duplicates.
...
- **28 Fundamental Beliefs UX:** Belief cards feature animated chevron rotation (150ms), scripture navigation with verse-specific gold highlight fade (500ms hold + 1s fade via state-stepped animation), visual authority hierarchy (scripture rows use Inter_600SemiBold with subtle accent background; EGW button uses reduced D9 opacity), and lightweight viewed-tracking via AsyncStorage with green checkmark badge on the belief number circle.
- **Great Controversy Timeline Engine:** An immersive vertical timeline tracing the cosmic conflict from Creation to the New Earth through 15 narrative nodes aligned with Adventist theology. Features: horizontal scrollable phase navigation bar (11 phases with auto-scroll tracking), compact color-coded legend (Scripture/History/Prophecy/Belief/Hope), conflict thread panels explaining cosmic-conflict significance per node, EGW links to egwwritings.org (visually subordinate), deep-linking CTAs ("Read key passage", "Explore prophecy", "Open belief", "Begin study"), AsyncStorage progress tracking (`gc_timeline_progress`) with viewed-node checkmarks on phase chips and last-phase restoration on return. Entry point from Explore tab.
- **Shared Scripture Navigation (`lib/scripture-nav.ts`):** Centralized helper for parsing scripture references and navigating to the Bible reader with verse highlighting. Used by Fundamental Beliefs, Great Controversy, and Verse Map screens. Handles formats like "Genesis 1:1", "Daniel 2:38-44", "Revelation 12:7-9".
