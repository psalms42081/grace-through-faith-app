...
- **Content Engine & Resources Library:** Two-stage content pipeline architecture:
  ...
  - **Diff View:** `resources.previous_content_json` JSONB column preserves old content on regeneration. `GET /api/admin/pipeline/resource/:id/diff` (requireEditor) returns section-by-section comparison (overview, daily study prompts, discussion questions, memory verse guide, family worship, EGW connections) with status badges (changed/unchanged/added/removed), generation metadata diff (prompt version, model, tokens), and summary counts. Diff endpoint falls back to loading content from superseded resource if `previousContentJson` is null. UI shows diff button (git-compare icon, yellow) on items with previous versions, opening a full-screen diff modal with collapsible sections — changed sections expanded by default, stacked old/new content blocks.
  - **Version-Safe Regeneration:** `resources.supersedes_resource_id` VARCHAR column links new drafts to the resource they replace. Regeneration flow: old published companion stays live, new draft is generated with `supersedesResourceId` pointing to old resource, old content captured in `previousContentJson` for fast diffing. On approval/publish, the superseded resource is archived (status='archived', reviewStatus='archived'). Archived resources excluded from public queries and admin review lists. Guard: only one pending regenerated draft allowed per lesson — stale drafts are removed before creating new ones.
  - **Auto-Generation on Sync:** When the 24-hour sync detects changed lesson content, source packets are rebuilt and hash-compared. Changed packets automatically trigger companion regeneration using the version-safe flow: new draft created, linked to old published version via `supersedesResourceId`, old content preserved for diffing. New companions appear in the editorial review queue as pending drafts with "Regenerated" badge. Never auto-publishes — editors must review and approve. The sync-triggered path (`triggerCompanionGeneration` in `sabbath-school-sync.ts`) mirrors the batch generator's version-safe logic exactly.
