...
- **Map Search & Focus System:** Unified search bar on Bible Maps screen (below overlay controls). Searches across all 6 entity types: BiblicalLocation (name, ancientRegion, modernLocation), BiblicalPeopleGroup (name, regionLabel), BiblicalProphecyLink (title, theme), BiblicalJourneyRoute (title, category), BiblicalKingdomOverlay (name, eraLabel), BiblicalTribeOverlay (name, regionLabel). Pre-built search index (`SEARCH_INDEX`) with title-first ranking. Active overlay type gets priority in results (e.g., if overlay=tribes, tribe results sort first). Results panel shows compact rows with type icon, title, subtitle, and colored type badge (Location/People Group/Prophecy/Journey/Kingdom/Tribe). Tapping a result navigates to the appropriate detail screen with full state preservation (mode, era, overlay, journey, kingdom, tribe). Empty state shows "No matching map results". Clear button dismisses search. Search query persists until manually cleared.
- **Related Studies Integration Layer:** Reusable `RelatedStudiesSection` component (`components/RelatedStudiesSection.tsx`) used across all 6 Bible Maps detail screens (Location, People Group, Prophecy, Journey Route, Kingdom, Tribe). Shows contextual study actions: Read Passages (routes to `/read/[bookId]/[chapter]`), Start Guided Study (routes to `/study-guide` with passage params), Start Deep Study (routes to `/deep-study-picker` with passage params), Open Historic Voices (routes to `/historic-voices` with book/chapter/bookName params), Open Prophecy Explorer (routes to `/prophecy-hub`, shown on prophecy/kingdom/location screens with prophecy connections), View on Map (routes to `/maps-timeline` with overlay context), View on Timeline (routes to maps with era context, for entities with timeline data). Actions are conditionally rendered based on entity data. Passage parsing centralized via exported `parsePassageReference()` function. Consolidates/replaces per-screen action sections — no duplicate action buttons.
...
- **Tribes of Israel Overlay:** `BiblicalTribeOverlay` type (id, name, regionLabel, shortDescription, keyPassages, relatedLocationIds, color, centerLatitude, centerLongitude, periods). `relatedTribeIds` field on `BiblicalLocation`. 7 tribes: Judah (amber), Benjamin (red), Ephraim (blue), Manasseh (purple), Dan (cyan), Naphtali (green), Levi (pink). Overlay selector now 6 options (None / People Groups / Prophecy / Journey Routes / Kingdoms / Tribes of Israel). When Tribes selected, map shows small Circle + label markers at each tribe's center. List below map shows tribe cards. Detail screen at `/tribe-overlay/[id]` with region badge, description, key passages (tappable), related locations, historical significance periods, and 2 action buttons (Read Passages, View on Map). Location detail screens show "Tribes of Israel" section. `getTribeById()`, `getTribesForLocation()` exported. Location wiring: Jerusalem (Judah, Benjamin, Levi), Bethlehem (Judah), Nazareth/Sea-of-Galilee/Capernaum (Naphtali), Jordan River (Manasseh). State preserved: mode + era + overlay + tribe across all navigation.
