...
- **Church Connect:** A global SDA church finder with 219 churches (130 Victorian churches with official addresses from the Victorian Conference directory + 89 international/interstate churches) across 40+ countries, geo-radius search (Haversine), text search by city/state/country, and map/list views. Seeded via `scripts/seed-global-churches.ts` on server startup — Victorian churches are refreshed from official data on each startup (delete + re-insert), other regions use case-insensitive dedup. DB has a unique index on `LOWER(name), LOWER(country)` to prevent duplicates. Victorian churches organized by official conference regions: Metro East, Metro South East, Metro West, Metro North, Central Melbourne, Regional East, Regional North East, Regional North West, Regional South West, Regional West.
